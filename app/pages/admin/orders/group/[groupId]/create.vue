...
const productSimpleList = useState<GroupBuyingProduct[]>(
    `productListByGroupId-${groupId.value}`,
    () => []
)
...
const getItems = (currentRowIndex: number) => {
    ...
    return productSimpleList.value.flatMap((productSimple: GroupBuyingProduct) => {
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    if (event.data) {
        ...
        await orderStore.createGroupOrder(groupId.value, submitData)
        ...
        await navigateTo({
            path: `/admin/orders/group/${groupId.value}`,
            query: { page: page.value || 1, unitName: unitName.value, endDate: endDate.value }
        })
    }
}
</script>
