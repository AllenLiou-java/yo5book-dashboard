            ...
            >
                <template #status-cell="{ row }">
                    ...
                    <div v-else class="flex items-center gap-2">
                        ...
                        <span>{{
                            getStatusName(String(row.original.status || '')) ||
...
await callOnce('initOrders', async () => {
    await orderStore.fetchPersonalOrders()
})
...
watchPostEffect(() => {
    // 建立依賴：當編輯列或展開狀態改變時觸發
    const _editing = editingRowId.value
...
const filteredOrders = computed(() => {
    ...
    return personalOrderList.value.filter((order) => {
        switch (searchField.value) {
            ...
            case 'address':
                return order.receiver?.address?.toLowerCase().includes(keyword)
            case 'name':
                return order.receiver?.name?.toLowerCase().includes(keyword)
            case 'phone':
                return order.phone?.toLowerCase().includes(keyword)
            case 'email':
                return order.email?.toLowerCase().includes(keyword)
            case 'bankAccountNo':
                return order.bankAccountNo?.toLowerCase().includes(keyword)
            case 'userId':
